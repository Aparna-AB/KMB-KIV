...
import CertoificateOfSurvey from "./components/certificateofSurveyCategoryA";
import "./style.scss";

const Index = () => {
  const [show, setShow] = useState(false);
  const [showCertificate, setShowCertificate] = useState(false);
  const [showCertificateB, setShowCertificateB] = useState(false);
  const [dataTable, setDataTable] = useState([]);
  const handleClose = () => setShow(false);
  const handleCloseCertificate = () => setShowCertificate(false);
  const handleCloseCertificateB = () => setShowCertificateB(false);
  ...
  return (
    <Container>
      ...
      {showCertificateB && (
        <CertoificateOfSurvey
          show={showCertificateB}
          handleClose={handleCloseCertificateB}
        />
      )}
